...
def extract_image_context_system_message(images: List[Dict[str, str]]) -> Dict[str, Any]:
    image_list = '\n'.join([f"{img['name']} {img['url']}" for img in images])
    
    return {
        'role': 'system',
        'content': f"""Extract contextual information for images mentioned in a user-provided article, focusing on details that enhance understanding of each image, and return it as an array of JSON objects.
...
preview_image_system_message: Dict[str, Any] = {
...
refine_description_system_message: Dict[str, Any] = {
