...
class TextSplitter:
    ...
    async def split(self, text: str, limit: int) -> List[IDoc]:
        ...
        while position < total_length:
            ...
            chunk_result = self._get_chunk(text, position, limit)
            ...
            url_image_result = self._extract_urls_and_images(chunk_text)
            
            chunks.append({
                'text': url_image_result['content'],
                'metadata': {
                    'tokens': tokens,
                    'headers': dict(current_headers),
                    'urls': url_image_result['urls'],
                    'images': url_image_result['images'],
                }
            })
    ...
    def _get_chunk(self, text: str, start: int, limit: int) -> Dict[str, Any]:
        ...
        if remaining_text:
            tokens_per_char = self._count_tokens(remaining_text) / len(remaining_text)
            estimated_chars = int(limit / tokens_per_char)
    ...
    def _extract_urls_and_images(self, text: str) -> Dict[str, Any]:
        urls: List[str] = []
        images: List[str] = []
        url_index = 0
        image_index = 0
    ...
    async def document(self, text: str, model: Optional[str] = None, additional_metadata: Optional[Dict[str, Any]] = None) -> IDoc:
        ...
        url_image_result = self._extract_urls_and_images(text)
